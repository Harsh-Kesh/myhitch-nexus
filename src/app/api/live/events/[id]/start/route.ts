// POST /api/live/events/[id]/start — the real "Go Live Now" transition, ownership-checked
// against the event's own channel (see liveEvents.ts's header for why this didn't exist).
import { NextResponse, type NextRequest } from "next/server";
import { startLiveEvent } from "@/lib/server/liveEvents";
...
export async function POST(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  const result = await startLiveEvent(account.id, id);
