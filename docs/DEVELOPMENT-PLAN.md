...
- ✅ **2026-09-14**: Typesense provisioned — self-hosted on Railway (service `typesense`, official Docker image, persistent volume at `/data`) rather than Typesense Cloud, avoiding a third vendor account for something easy to self-host. One real gotcha worth recording: the default 384-thread pool crash-looped the container ("Resource temporarily unavailable") until `TYPESENSE_THREAD_POOL_SIZE=8` was set — its naive CPU-based default doesn't account for container resource limits. `GET /api/videos` now does the **full faceted search** from `docs/openapi.yaml` (content type, category, language, country, access model, age rating, duration range, release-year range, sort) via `src/lib/server/typesense.ts` + `scripts/index-catalogue.mjs` (one-way sync from Postgres, source of truth stays Postgres — the index is rebuilt from scratch on each run, hydrated back into full rows by id afterward so a stale reindex can only affect ranking, never wrong data). Verified with combined filter queries against the real index. `sort=popular`/`sort=rating` are accepted but fall back to newest — no real view/rating data exists yet, and faking it would be worse than being explicit about the gap.
- Still to do: Auth0 integration (headless ROPG, per the standard) replacing the `sessionStorage` mock — blocked, resumes when the shared-tenant work lands; server-side RBAC foundation; the missing **Creators directory** (FR-6.1.2); watch progress/watchlist/follows/comments/ratings write paths (tables exist, no routes yet — all need an authenticated `account_id`, so blocked with Auth0); OpenGraph/JSON-LD metadata (FR-6.1.7).
...
6. **Mux account** (media/live vendor, confirmed 2026-09-14 — see the architecture table above) — needs a real account and eventually a payment method to get an Access Token ID + Secret Key. Nothing in P2 (upload/transcode) or P5 (live) can start against the real vendor until this exists; everything else keeps moving in the meantime.
