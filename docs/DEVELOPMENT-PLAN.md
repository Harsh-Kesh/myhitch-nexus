...
- ✅ **2026-09-14**: `getCategories()` in `src/lib/mock-api/index.ts` is the **first mock-api function actually swapped to real data**, per the strangler-fig strategy in §2 — same signature, now calls `fetch('/api/categories/')` instead of the in-memory store. Chosen specifically because `Category`'s fields are at full parity with what the real table returns.
  ...
  - `getVideo`/`getChannel`/`searchVideos` remain on mock for a different, already-known reason: those types carry engagement counters (views/likes/ratings) and a couple of Channel fields (languages, links) that don't exist in the real schema yet (by design — they're analytics-pipeline/SRS-§6.9 outputs). Swap them once that gap is deliberately closed.
- ⏳ **In progress**: seeding the catalogue tables from the prototype's actual mock data (`scripts/seed-catalogue.mjs`, reusing the real mock objects programmatically rather than hand-transcribing) so the real API has real content to serve.
- Still to do: Auth0 integration (headless ROPG, per the standard) replacing the `sessionStorage` mock — blocked, resumes when the shared-tenant work lands; server-side RBAC foundation; Typesense index (current `GET /api/videos` does a real but intentionally partial Postgres-only filter — see the comment in `catalogue.ts` — full faceted search waits for Typesense); the missing **Creators directory** (FR-6.1.2); watch progress/watchlist/follows/comments/ratings write paths (tables exist, no routes yet — all need an authenticated `account_id`, so blocked with Auth0); OpenGraph/JSON-LD metadata (FR-6.1.7).
...
6. Stand up dev/test/staging environments (DEL-5) and restructure the repo to the monorepo layout — **deferred deliberately**: premature before a second service (e.g. a worker) actually exists to justify it; revisit at the start of whichever phase first needs one.
7. Provision Sentry, Redis, Typesense — worth doing once there's a feature that needs them; Sentry specifically could go on the live site any time.
8. **In progress (2026-09-14)**: generating the OpenAPI specification from the existing ~120 mock-api signatures (DEL-4), covering every entity in SRS-TRACEABILITY.md §G and grouped by the same IA sections as §D.
9. **Done**: the four Auth0/identity lib files (`auth0Client.ts`/`auth0Sync.ts`/`auth0Mfa.ts`/`socialIdentity.ts`) are written per the standard and typecheck clean; `accounts.auth0_user_id` is live. Remaining before blocker #2 lands: our own register/login/reset **routes** that call this layer, session issuance, rate limiting, the own-OTP verification flow, and the server-side RBAC authorization layer reading `account_roles` — all buildable and unit-testable against placeholder env vars now. Only the real end-to-end login (an actual Auth0 tenant to call) waits on tenant access.
10. **Done (2026-09-14)**: catalogue schema, seed data, first real API routes (`GET /api/videos`, `/api/videos/{id}`, `/api/categories`, `/api/channels/{id}`, `/api/channels/{id}/videos`), and the Explore page's category grid genuinely running off Postgres — see the P1 entry above for what was and wasn't swapped, and why.
