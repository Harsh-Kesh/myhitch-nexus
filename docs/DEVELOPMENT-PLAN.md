...
- Still to do: real Auth0 integration (the seam above is what makes that swap small when tenant access lands — blocker #2 below); the actual watchlist/ratings/comments/watch-progress write routes (tables exist, `rbac.ts`'s `accountHasRole`/`getRequestAccount` are ready for them, no routes yet); the end-to-end test harness (`npm run test:e2e`) is still non-functional (see the finding at the bottom of §9); remove the `looksLikeRealId` bridge once the home rails and category pages migrate off the mock store.
