...
- Auth0 integration (headless ROPG, per the standard) replacing the `sessionStorage` mock: our own register/login/verify/reset routes calling `auth0Sync.ts` server-side, our own session issued on success, real sessions surviving reload, Next.js middleware route protection.
