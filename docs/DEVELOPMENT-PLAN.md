...
- Still to do: Auth0 integration (headless ROPG, per the standard) replacing the `sessionStorage` mock — blocked, resumes when the shared-tenant work lands; server-side RBAC foundation; watch progress/watchlist/follows/comments/ratings write paths (tables exist, no routes yet — all need an authenticated `account_id`, so blocked with Auth0); the end-to-end test harness (`npm run test:e2e`) is still non-functional (see the finding at the bottom of §9); remove the `looksLikeRealId` bridge once the home rails and category pages migrate off the mock store.
