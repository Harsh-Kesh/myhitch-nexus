...
4. **Apple Developer Program membership** (FR-6.2.1 follow-through) — USD 99/year, required just to generate Sign-in-with-Apple credentials. A cost decision, not a coordination one, so it's grouped here rather than chased separately. **Not blocking**: Google sign-in (free, self-serve) proceeds in P1 regardless.
...
6. Provision Sentry, Redis, Typesense.
7. Generate the OpenAPI specification from the existing ~120 mock-api signatures (DEL-4) and circulate for review.
8. Create the Google OAuth client (Google Cloud Console) — free, doesn't need to wait for the meeting above.
