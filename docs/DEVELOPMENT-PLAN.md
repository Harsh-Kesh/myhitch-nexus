...
- Still to do: real Auth0 integration (the seam in the local-sign-in entry above is what makes that swap small when tenant access lands — blocker #2 below); remove the `looksLikeRealId` bridge once the homepage itself migrates off the mock store (now the only browse-side page left on it); Sentry provisioning is next but needs a Sentry account/DSN — that's a signup only the client can do, not something to create on their behalf.
