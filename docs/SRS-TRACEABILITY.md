...
| ID | Requirement | Prototype | Build approach | Phase |
|---|---|---|---|---|
| FR-6.2.1 | Email registration, secure login, password reset, optional social sign-in | ✅ UI (mock: any password works) | **Headless** Auth0 (docs/AUTH0_INTEGRATION_STANDARD.md) — Resource Owner Password Grant via our own backend, never a hosted-login redirect; our own OTP-based reset flow, with only the actual password write going through Auth0's Management API. **Social sign-in**: resolved as one flow with two front doors — `socialIdentity.ts` verifies a Google/Apple ID token directly against that provider's JWKS (no Auth0, no redirect), then converges into the same Auth0 identity/session path via `findOrCreateAuth0UserForVerifiedEmail()`. **Google ships in P1** (free, self-serve credential). **Apple is gated on an Apple Developer Program membership (USD 99/year)** — a cost decision in the blocker list, not a technical one. | P1 (Google), pending decision (Apple) |
...
| FR-6.8.6 | ~~Administrative approval before advertisements go live~~ **Superseded 2026-09-25, client policy** — no platform staff in ordinary product flows; an advertiser's own submission triggers real automated technical checks (file validity, malware scan) instead | ✅ **Live, automated, server-enforced** — `POST /api/campaigns/[id]/submit` (advertiser-only) auto-approves or auto-rejects based on real ffprobe/malware checks; `decideCampaign()`/`PATCH /api/campaigns/[id]/status` (`moderator`/`super-admin`) remain as a reactive override on an already-live campaign, not a required gate before one can go live | `autoActivateCampaign()`/`decideCampaign()` in `campaigns.ts` | P6 |
...
| DEC-6 | MYHitch identity: shared login with main MYHitch platform or separate | Shared — Auth0 is already the identity provider for the other MYHitch platforms (Pass confirmed); Nexus registers as a new Application inside that *existing* tenant, following docs/AUTH0_INTEGRATION_STANDARD.md (headless ROPG, extracted from Pass's real implementation) | TPI-8, all auth work |
